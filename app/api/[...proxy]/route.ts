...
const ROUTES: RouteConfig[] = [
    ...
    // Prefixed Routes (Recommended)
    {
        ...
        host: 'api.openai.com',
        name: 'OpenAI',
        rewrite: (path: string) => path.replace(/^openai\//, '').replace(/^(?!v1\/)/, 'v1/')
    },
    {
        pattern: /^anthropic\//,
        host: 'api.anthropic.com',
        name: 'Anthropic',
        rewrite: (path: string) => path.replace(/^anthropic\//, '').replace(/^(?!v1\/)/, 'v1/')
    },
    {
        pattern: /^gemini\//,
        host: 'generativelanguage.googleapis.com',
        name: 'Gemini',
        rewrite: (path: string) => path.replace(/^gemini\//, '').replace(/^(?!v1beta\/)/, 'v1beta/')
    },
    {
        pattern: /^groq\//,
        host: 'api.groq.com',
        name: 'Groq',
        rewrite: (path: string) => path.replace(/^groq\//, '').replace(/^(?!openai\/v1\/)/, 'openai/v1/')
    },
    {
        pattern: /^openrouter\//,
        host: 'openrouter.ai',
        name: 'OpenRouter',
        rewrite: (path: string) => path.replace(/^openrouter\//, '').replace(/^(?!api\/v1\/)/, 'api/v1/')
    },

    // Legacy/Direct Routes (Maintained for compatibility where safe)
...
const handleRequest = async (req: NextRequest, pathSegments: string[]): Promise<NextResponse> => {
    ...
    const rewrittenPath = route.rewrite ? route.rewrite(fullPath) : fullPath;
    const targetUrl = new URL(`https://${route.host}/${rewrittenPath}`);
